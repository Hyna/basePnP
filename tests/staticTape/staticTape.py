...
firstPartPos = (25,463) # normally determined by user by clicking into the image.
currentPartPos =  (0,0) 
...
img = cv2.imread('tape2.png',0) # CCD image
...
for x in range(2,partCount+1):
	...
	if tapeOrientation == 0:
		currentPartPos = (previousPartPos[0]-122, previousPartPos[1])
	elif tapeOrientation == 1:
		currentPartPos = (previousPartPos[0], previousPartPos[1]+122)
	...
	print "Part %d is on pos X: %d Y: %d" % (x, currentPartPos[0], currentPartPos[1])
    	...
    	cv2.line(cimg2, (currentPartPos[0], currentPartPos[1]-25), (currentPartPos[0], currentPartPos[1]+25), crossColor,crossThickenss) # vertical line
